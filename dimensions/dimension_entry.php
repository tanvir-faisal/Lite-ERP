...
page(_($help_context = "Meter Usage"), false, false, "", $js);
...
function can_process()
{
	...
	if ($selected_id == -1) 
	{
    	...
    	{
    		display_error( _("The meter no must be entered."));
			set_focus('ref');
    		return false;
    	}
    	...
    	{
    		display_error(_("The entered meter no is already in use."));
			set_focus('ref');
    		return false;
    	}
	}
	...
	if (strlen($_POST['meter']) == 0) 
	{
		display_error( _("The meter reading must be entered."));
		set_focus('meter');
		return false;
	}
...
if (isset($_POST['ADD_ITEM']) || isset($_POST['UPDATE_ITEM'])) 
{
	...
	if (can_process()) 
	{
		...
		if ($selected_id == -1) 
		{
			$id = add_dimension($_POST['ref'], $_POST['name'], $_POST['meter'], $_POST['type_'], $_POST['date_'], $_POST['due_date'], $_POST['memo_']);
		...
		else 
		{
			...
			update_dimension($selected_id, $_POST['name'], $_POST['meter'], $_POST['type_'], $_POST['date_'], $_POST['due_date'], $_POST['memo_']);
...
if (isset($_POST['delete'])) 
{
	...
	{
		display_error(_("This meter reading cannot be deleted because it has already been processed."));
		set_focus('ref');
		$cancel_delete = true;
	}
...
if ($selected_id != -1)
{
	...
	if (strlen($myrow[0]) == 0) 
	{
		display_error(_("The meter reading sent is not valid."));
		display_footer_exit();
	}

	// if it's a closed meter reading can't edit it
	if ($myrow["closed"] == 1) 
	{
		display_error(_("This meter reading is closed and cannot be edited."));
		display_footer_exit();
	}
...
{
	$_POST['dimension_tags'] = array();
	ref_row(_("ID:"), 'ref', '', $Refs->get_next(ST_DIMENSION));
}

text_row_ex(_("Meter No") . ":", 'name', 11, 11);

text_row_ex(_("Meter Reading") . ":", 'meter', 11, 11);
...
if (isset($_POST['closed']) && $_POST['closed'] == 1)
	display_note(_("This Meter Reading is closed."), 0, 0, "class='currentfg'");

if ($selected_id != -1) 
{
	echo "<br>";
	submit_center_first('UPDATE_ITEM', _("Update"), _('Save changes to Meter Reading'), 'default');
	if ($_POST['closed'] == 1)
		submit('reopen', _("Re-open This Meter Reading"), true, _('Mark this Reading as re-opened'), true);
	else	
		submit('close', _("Close This Meter Reading"), true, _('Mark this Meter Reading as closed'), true);
	submit_center_last('delete', _("Delete This Meter Reading"), _('Delete unused Meter Reading'), true);
}
else
{
